...
export default function EditQuotation() {
  ...
  const loadData = async () => {
    try {
      const [batchesData, customersData, quotationData] = await Promise.all([
      ...
      setQuotationDate(quotationData.quotationDate
        ? new Date(quotationData.quotationDate).toISOString().split('T')[0] : '');
      setValidityDate(quotationData.validityDate
        ? new Date(quotationData.validityDate).toISOString().split('T')[0] : '');

      // Map existing items to match the form structure
      ...
      if (quotationData.customer) {
        const matched = customersData.find(c => c._id === (quotationData.customer?._id || quotationData.customer));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const totalCGST = taxType === 'CGST_SGST' ? totals.totalTax / 2 : 0;
      const totalSGST = taxType === 'CGST_SGST' ? totals.totalTax / 2 : 0;
      const totalIGST = taxType === 'IGST' ? totals.totalTax : 0;
    ...
    <DashboardLayout>
      <div className="max-w-5xl mx-auto">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-gray-600">Update the quotation details</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 text-black">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="flex gap-2">
                        ...
                        <div className="p-2 border-b border-gray-200">
                          <div className="relative">
                            ...
                            <input type="text" placeholder="Search..." value={customerSearchTerm}
                              ...
                              className="w-full pl-9 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" autoFocus />
                          </div>
                        </div>
                  ...
                  <button type="button" onClick={() => setShowCustomerModal(true)} className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button type="button" onClick={addItem} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">
                + Add Item
              </button>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              ...
              {totals.totalTax > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">{taxType === 'CGST_SGST' ? 'CGST + SGST' : 'IGST'}:</span>
              ...
              <div className="pt-3 border-t border-gray-200">
                <div className="flex justify-between text-lg font-bold">
                  ...
                  <span className="text-indigo-600">₹{totals.finalTotal.toLocaleString('en-IN')}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-black">
            {[['name','Customer Name',true,'text'],['phone','Phone',true,'tel'],['email','Email',false,'email'],['gstin','GSTIN',false,'text']].map(([field, label, required, type]) => (
              <div key={field}>
                ...
                <input type={type} name={field} value={customerFormData[field]}
                  ...
                  className={`w-full px-4 py-2 border rounded-lg focus:ring-2 ${customerFormErrors[field] ? 'border-red-500 focus:ring-red-500 bg-red-50' : 'border-gray-300 focus:ring-emerald-500'}`} />
            ...
            {['city','state','pincode'].map(field => (
              <div key={field}>
                ...
                <input type="text" name={field} value={customerFormData[field]}
